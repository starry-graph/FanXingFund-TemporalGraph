...
def test_model(test_dataloader, model, device, loss_fcn, epoch, args):
    ...
    with torch.no_grad():
        ...
        print(
            f"F1-Score on testset:       whole {mean_score:.4f}, part1 {mean_score_part1:.4f}, part2 {mean_score_part2:.4f}")

        res_path4 = "logs/testScore/{}-{}.csv".format(args.loss_method, args.lr)
        headers4 = ["epoch", "mean_score", "mean_score_part1", "mean_score_part2"]
        if not os.path.exists(res_path4):
            f = open(res_path4, 'w+')
            f.write(",".join(headers4) + "\r\n")
            f.close()
            os.chmod(res_path4, 0o777)
        with open(res_path4, "a") as file:
            file.write("{:05d},{:.4f},{:.4f},{:.4f}".format(
                epoch, mean_score, mean_score_part1, mean_score_part2))
            file.write("\n")
...
def val_model(valid_dataloader, model, device, loss_fcn, epoch, args):
    ...
    with torch.no_grad():
        ...
        print(
            f"F1-Score on valset:       whole {mean_score:.4f}, part1 {mean_score_part1:.4f}, part2 {mean_score_part2:.4f}")

        res_path3 = "logs/validScore/{}-{}.csv".format(args.loss_method, args.lr)
        headers3 = ["epoch", "mean_score", "mean_score_part1", "mean_score_part2"]
        if not os.path.exists(res_path3):
            f = open(res_path3, 'w+')
            f.write(",".join(headers3) + "\r\n")
            f.close()
            os.chmod(res_path3, 0o777)
        with open(res_path3, "a") as file:
            file.write("{:05d},{:.4f},{:.4f},{:.4f}".format(
                epoch, mean_score, mean_score_part1, mean_score_part2))
            file.write("\n")
...
def get_data_loader(args, type, device):
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, collate_fn=collate, num_workers=4,
                                  shuffle=True)

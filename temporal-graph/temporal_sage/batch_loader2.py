...
class TemporalEdgeCollator:
    ...
    def _collate_with_negative_sampling(self, items):
        ...
        if self.dgl_sampler:
            ...
            history_gs = [self.time_range[i] for i in range(start_year, min_year)]
        dhistory_gs = [self.graphs[i] for i in range(start_year, min_year)]
        ...
        for g_sampling in  history_gs:
            ...
            history_blocks.append(self.block_sampler[0].sample_blocks(
                g_sampling, seed_nodes
            ))
        input_nodes = [blocks[0].srcdata[dgl.NID] for blocks in history_blocks]
        
        sampler, dsampler = self.block_sampler[0], self.block_sampler[1]
        year_range, dg = history_gs[0], dhistory_gs[0]
        # dyear_range = (dg.edata['ts'].min(), dg.edata['ts'].max())
        block = history_blocks[0]
        dblock = dsampler.sample_blocks(dg, seed_nodes)
        
        ngh = sampler.sample_neighbors(year_range, seed_nodes, 15, 100)
        dngh = dgl.sampling.sample_neighbors(dg, seed_nodes, 15)
        
        src, dst = ngh.edges()
        dsrc, ddst = dngh.edges()
        a = torch.tensor(sorted(dst))
        b = torch.tensor(sorted(ddst))
